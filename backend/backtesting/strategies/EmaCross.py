...
class EmaCross(bt.Strategy):
    ...
    def __init__(self, ema1=10, ema2=30, target=0.05, stoploss=0.01):

        self.ema1 = bt.ind.EMA(period=ema1)
        self.ema2 = bt.ind.EMA(period=ema2)
    ...
    def next(self):
        ...
        if not self.position:  # not in the market
            ...
            # Positive ema cross from time before
            ...
            # Negabive ema cross from time before
            elif self.ema1[0] < self.ema2[0] and self.ema1[-1] > self.ema2[-1]:
                entry_price = self.data.close[0]
                size = 10  # or however much you want
